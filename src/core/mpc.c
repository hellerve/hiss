...
static void mpc_err_add_expected(mpc_err_t *x, char *expected) {
  ...
  x->expected_num++;
  x->expected = realloc(x->expected, sizeof(char*) * (unsigned) x->expected_num);
...
static mpc_err_t *mpc_err_count(mpc_err_t *x, int n) {
  mpc_err_t *y;
  int digits = n/10 + 1;
  char *prefix = malloc((unsigned) digits + strlen(" of ") + 1);
...
static void mpc_input_mark(mpc_input_t *i) {
  ...
  i->marks_num++;
  i->marks = realloc(i->marks, sizeof(mpc_state_t) * (unsigned) i->marks_num);
  i->lasts = realloc(i->lasts, sizeof(char) * (unsigned) i->marks_num);
...
static void mpc_stack_parsers_reserve_more(mpc_stack_t *s) {
  if (s->parsers_num > s->parsers_slots) {
    s->parsers_slots = (int) ceil((s->parsers_slots+1) * 1.5);
    s->parsers = realloc(s->parsers, sizeof(mpc_parser_t*) * (unsigned) s->parsers_slots);
    s->states = realloc(s->states, sizeof(int) * (unsigned) s->parsers_slots);
  }
}

static void mpc_stack_parsers_reserve_less(mpc_stack_t *s) {
  if (s->parsers_slots > pow(s->parsers_num+1, 1.5)) {
    s->parsers_slots = (int) floor((s->parsers_slots-1) * (1.0/1.5));
    s->parsers = realloc(s->parsers, sizeof(mpc_parser_t*) * (unsigned) s->parsers_slots);
    s->states = realloc(s->states, sizeof(int) * (unsigned) s->parsers_slots);
  }
}
...
static void mpc_stack_results_reserve_more(mpc_stack_t *s) {
  if (s->results_num > s->results_slots) {
    s->results_slots = (int) ceil((s->results_slots + 1) * 1.5);
    s->results = realloc(s->results, sizeof(mpc_result_t) * (unsigned) s->results_slots);
    s->returns = realloc(s->returns, sizeof(int) * (unsigned) s->results_slots);
  }
}

static void mpc_stack_results_reserve_less(mpc_stack_t *s) {
  if ( s->results_slots > pow(s->results_num+1, 1.5)) {
    s->results_slots = (int) floor((s->results_slots-1) * (1.0/1.5));
    s->results = realloc(s->results, sizeof(mpc_result_t) * (unsigned) s->results_slots);
    s->returns = realloc(s->returns, sizeof(int) * (unsigned) s->results_slots);
  }
}
...
void mpc_cleanup(int n, ...) {
  int i;
  mpc_parser_t **list = malloc(sizeof(mpc_parser_t*) * (unsigned) n);
...
mpc_parser_t *mpc_or(int n, ...) {
  ...
  p->type = MPC_TYPE_OR;
  p->data.or.n = n;
  p->data.or.xs = malloc(sizeof(mpc_parser_t*) * (unsigned) n);
...
mpc_parser_t *mpc_and(int n, mpc_fold_t f, ...) {
  ...
  p->data.and.n = n;
  p->data.and.f = f;
  p->data.and.xs = malloc(sizeof(mpc_parser_t*) * (unsigned) n);
  p->data.and.dxs = malloc(sizeof(mpc_dtor_t) * (unsigned) (n-1));
...
mpc_ast_t *mpc_ast_add_child(mpc_ast_t *r, mpc_ast_t *a) {
  r->children_num++;
  r->children = realloc(r->children, sizeof(mpc_ast_t*) * (unsigned) r->children_num);
...
static mpc_parser_t *mpca_grammar_find_parser(char *x, mpca_grammar_st_t *st) {
  ...
  if (is_number(x)) {
    ...
    while (st->parsers_num <= i) {
      st->parsers_num++;
      st->parsers = realloc(st->parsers, sizeof(mpc_parser_t*) * (unsigned) st->parsers_num);
...
static mpc_val_t *mpca_stmt_fold(int n, mpc_val_t **xs) {
  ...
  int i;
  mpca_stmt_t **stmts = malloc(sizeof(mpca_stmt_t*) * (unsigned) (n+1));
